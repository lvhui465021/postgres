...
typedef struct ColumnChunkSkipNode
{
	...
	/*
	 * Offsets and sizes of value and exists streams in the column data. These
	 * enable us to skip reading suppressed row chunks, and start reading a
	 * chunk without reading previous chunks.
	 */
...
typedef struct ChunkData
{
	...
	/*
	 * valueBuffer keeps actual data for type-by-reference datums from
	 * valueArray.
	 */
...
typedef enum StripeWriteStateEnum
{
	...
	/*
	 * Writer transaction is still in-progress. Note that it is not certain if
	 * it is being written by current backend's current transaction or another
	 * backend.
	 */
...
{
	slock_t		mutex;
	pg_atomic_uint64 nextStripeId;	/* Fetch next stripe id to be read and
									 * increment */
...
extern void SaveStripeSkipList(RelFileLocator relfilelocator, uint64 stripe,
							   ...
							   TupleDesc tupleDescriptor);
extern void SaveStripeSkipListForStorageId(uint64 storageId, uint64 stripe,
										   StripeSkipList * stripeSkipList,
										   TupleDesc tupleDescriptor);
extern void SaveChunkGroups(RelFileLocator relfilelocator, uint64 stripe,
							List *chunkGroupRowCounts);
extern void SaveChunkGroupsForStorageId(uint64 storageId, uint64 stripe,
										List *chunkGroupRowCounts);
