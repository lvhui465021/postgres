...
{																			\
	...
	VectorFnArgument *right = (VectorFnArgument *) PG_GETARG_POINTER(1);	\
		...
		LTYPE *leftVectorValue = NULL;											\
		RTYPE *rightVectorValue = NULL;											\
		LTYPE leftConstValue = 0;												\
		RTYPE rightConstValue = 0;												\
		bool *vectorNull = NULL;												\
		bool *resIdx = NULL;													\
		bool *resNull = NULL;													\
		int i = 0;																\
																				\
		...
		{																		\
			...
			rightConstValue = right->arg;										\
																				\
			res = BuildVectorColumn(vectorColumn->dimension, 1, true, NULL);	\
																				\
			leftVectorValue = (LTYPE *) vectorColumn->value;					\
			vectorNull = (bool *) vectorColumn->isnull;							\
			resIdx = (bool *) res->value;										\
			resNull = (bool *) res->isnull;										\
																				\
			for (i = 0; i < vectorColumn->dimension; i++)						\
			{																	\
				resNull[i] = vectorNull[i];										\
				resIdx[i] = !vectorNull[i] && leftVectorValue[i] OPSYM rightConstValue; \
			}																	\
																				\
		...
		{																		\
			...
			leftConstValue = left->arg;											\
																				\
			res = BuildVectorColumn(vectorColumn->dimension, 1, true, NULL);	\
																				\
			rightVectorValue = (RTYPE *) vectorColumn->value;					\
			vectorNull = (bool *) vectorColumn->isnull;							\
			resIdx = (bool *) res->value;										\
			resNull = (bool *) res->isnull;										\
																				\
			...
			{																	\
				resNull[i] = vectorNull[i];										\
				resIdx[i] = !vectorNull[i] && rightVectorValue[i] OPSYM leftConstValue; \
			}																	\
																			\
		res->dimension = vectorColumn->dimension;							\
	}																		\
