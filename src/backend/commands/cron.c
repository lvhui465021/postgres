...
/*
 * cron_init_gucs registers all GUC variables for pg_cron.
 * Must be called before cron_init(), during postmaster startup.
 */
void
cron_init_gucs(void)
{
...
/*
 * cron_init registers the pg_cron launcher background worker and
 * relcache callback.  Must be called after cron_init_gucs() during
 * postmaster startup.
 */
void
cron_init(void)
...
PgCronLauncherMain(Datum arg)
{
	 ...
	 * Since UseBackgroundWorkers is always true in kernelized pg_cron,
	 * this is bounded solely by max_worker_processes. */
	if (max_worker_processes - 1 < MaxRunningTasks)
	{
 ...
 * 1 second, or until the next minute boundary.
 */
static void
WaitForCronTasks(List *taskList)
{
	...
	{
		/* Wait until the next minute boundary, but no more than 1 second */
		TimestampTz currentTime = GetCurrentTimestamp();
		TimestampTz nextMinute = TimestampMinuteEnd(currentTime);
		long waitSeconds = 0;
		int waitMicros = 0;
		int pollTimeout;

		TimestampDifference(currentTime, nextMinute,
							&waitSeconds, &waitMicros);
