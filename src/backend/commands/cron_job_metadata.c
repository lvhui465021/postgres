 ...
 *	  Adapted from pg_cron/src/job_metadata.c for kernel integration.
 *
 * Portions Copyright (c) 2016-2025, Citus Data, Inc.
 *
 * src/backend/commands/cron_job_metadata.c
 *
 *-------------------------------------------------------------------------
 */
...
static Oid CronJobRelationOid(void);
...
/* SQL-callable functions - registered via CREATE FUNCTION in cron.sql */
Datum cron_schedule(PG_FUNCTION_ARGS);
Datum cron_schedule_named(PG_FUNCTION_ARGS);
Datum cron_unschedule(PG_FUNCTION_ARGS);
Datum cron_unschedule_named(PG_FUNCTION_ARGS);
Datum cron_job_cache_invalidate(PG_FUNCTION_ARGS);
Datum cron_alter_job(PG_FUNCTION_ARGS);
				...
				text *usernameText, bool active, text *jobnameText)
{
	...
	appendStringInfo(&querybuf,
		...
		quote_qualified_identifier("pg_catalog", "pg_cron_job"));
...
/*
 * cron_schedule_named schedule a named job
 */
Datum
...
NextRunId(void)
{
	...
	/* generate new and unique run id from sequence */
...
/*
 * CronExtensionOwner returns the owner of the pg_cron extension, or
 * BOOTSTRAP_SUPERUSERID when running as a built-in kernel feature.
 */
static Oid
CronExtensionOwner(void)
{
	/* pg_cron is now a built-in kernel feature, not an extension.
	 * Use BOOTSTRAP_SUPERUSERID as the effective owner for privilege
	 * escalation during SPI operations.
	 */
...
cron_unschedule(PG_FUNCTION_ARGS)
{
	...
	cronSchemaId = get_namespace_oid("pg_catalog", false);
	cronJobIndexId = get_relname_relid("pg_cron_job_pkey", PG_CATALOG_NAMESPACE);

	cronJobsTable = table_open(CronJobRelationOid(), RowExclusiveLock);

	ScanKeyInit(&scanKey[0], 1,		/* Anum_cron_job_jobid */
...
cron_unschedule_named(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Handle the type change from NAME to TEXT for the jobname column,
	 * for backward compatibility with older function signatures.
	 */
...
EnsureDeletePermission(Relation cronJobsTable, HeapTuple heapTuple)
{
	...
	if (pg_strcasecmp(userName, ownerName) != 0)
	{
		...
		AclResult aclResult = pg_class_aclcheck(CronJobRelationOid(), GetUserId(),
												ACL_DELETE);
		if (aclResult != ACLCHECK_OK)
		{
			aclcheck_error(aclResult,
						   ...
						   OBJECT_TABLE,
#endif
						   get_rel_name(CronJobRelationOid()));
		}
	}
}
...
InvalidateJobCache(void)
{
	...
	classTuple = SearchSysCache1(RELOID, ObjectIdGetDatum(CronJobRelationOid()));
...
/*
 * CronJobRelationOid returns the OID of the pg_catalog.cron_job shared relation.
 */
static Oid
CronJobRelationOid(void)
{
	if (CachedCronJobRelationId == InvalidOid)
	{
		/* Use the BKI-assigned OID directly */
 ...
 * pg_catalog.cron_job shared relation.
 */
List *
LoadCronJobList(void)
{
	...
	/*
	 * If the cron_job table has not been created yet or
	...
	cronJobTable = table_open(CronJobRelationOid(), AccessShareLock);
...
TupleToCronJob(TupleDesc tupleDescriptor, HeapTuple heapTuple)
{
	...
	{
		/*
		 * Skip over superuser jobs when cron.enable_superuser_jobs
		 * is disabled.
		 */
...
InsertJobRunDetail(int64 runId, int64 *jobId, char *database, char *username, char *command, char *status)
{
	...
	if (RecoveryInProgress() || !JobRunDetailsTableExists())
	{
		PopActiveSnapshot();
		CommitTransactionCommand();
		MemoryContextSwitchTo(originalContext);
...
AlterJob(int64 jobId, text *scheduleText, text *commandText, text *databaseText, text *usernameText, bool *active)
{
	...
	if (RecoveryInProgress() || !JobTableExists())
	{
		return;
	}
	...
	appendStringInfo(&querybuf,
		"update %s.%s set", "pg_catalog", "pg_cron_job");
...
/*
 * JobTableExists returns whether the cron_job shared table exists.
 */
static bool
JobTableExists(void)
{
	/* Check by OID since it's a shared relation in pg_catalog */
	return SearchSysCacheExists1(RELOID, ObjectIdGetDatum(CronJobRelationId));
}
