							   ...
							   bool *snapshotRegisteredByUs)
{
	...
	/*
	 * If we flushed any pending writes, then we should guarantee that those
	 * writes are visible to us too. For this reason, if given snapshot is an
	 * MVCC snapshot, then we set its curcid to current command id.
	 *
	 * For simplicity, we do that even if we didn't flush any writes since we
	 * don't see any problem with that.
	 *
	 * XXX: We should either not update cid if we are executing a FETCH (from
	 * cursor) command, or we should have a better way to deal with pending
	 * writes, see the discussion in
	...
	/*
	 * To be able to use UpdateActiveSnapshotCommandId, we pushed the copied
	 * snapshot to the stack. However, we don't need to keep it there since we
	 * will anyway rely on ColumnarReadState->snapshot during read operation.
	 *
	 * Note that since we registered the snapshot already, we guarantee that
	 * PopActiveSnapshot won't free it.
	 */
	PopActiveSnapshot();
