...
static const struct CustomPathMethods ColumnarScanPathMethods = {
...
static const struct CustomScanMethods ColumnarScanScanMethods = {
	.CustomName = "ColumnarScan",
	.CreateCustomScanState = ColumnarScan_CreateCustomScanState,
};

static const struct CustomExecMethods ColumnarScanExecuteMethods = {
	.CustomName = "ColumnarScan",
...
static
PlannedStmt *
ColumnarPlannerHook(Query *parse, const char *query_string,
					int cursorOptions, ParamListInfo boundParams)
{
	PlannedStmt *stmt;
						   ...
						   RangeTblEntry *rte)
{
	...
	{
		/*
		 * some calls to the pathlist hook don't have a valid relation set. Do
		 * nothing
		 */
		return;
	}
	...
	/*
	 * Here we want to inspect if this relation pathlist hook is accessing a
	 * columnar table. If that is the case we want to insert an extra path
	 * that pushes down the projection into the scan of the table to minimize
	 * the data read.
	 */
	...
	if (relation->rd_tableam == GetColumnarTableAmRoutine())
	{
		...
		{
			/*
			 * When columnar custom scan is enabled
			 * (columnar.enable_custom_scan), we only consider
			 * ColumnarScanPath's & IndexPath's. For this reason, we remove
			 * other paths and re-estimate IndexPath costs to make accurate
			 ...
			 * when costing ColumnarCustomScan. In that case, if we don't
			 * remove SeqPath's, we might wrongly choose SeqPath thinking that
			 * its cost would be equal to ColumnarCustomScan.
			 */
							...
							bool inhparent, RelOptInfo *rel)
{
	...
	if (IsColumnarTableAmTable(relationObjectId))
	{
		...
		columnar_foreach_ptr(indexOptInfo, rel->indexlist)
		{
...
RemovePathsByPredicate(RelOptInfo *rel, PathPredicate removePathPredicate)
{
	...
	columnar_foreach_ptr(path, rel->pathlist)
	{
	...
	columnar_foreach_ptr(path, rel->partial_pathlist)
	{
		if (!removePathPredicate(path))
		{
...
CostColumnarPaths(PlannerInfo *root, RelOptInfo *rel, Oid relationId)
{
	...
	columnar_foreach_ptr(path, rel->pathlist)
	{
		...
		{
			/*
			 * Since we don't provide implementations for
			 * scan_bitmap_next_block & scan_bitmap_next_tuple, postgres
			 * doesn't generate bitmap index scan paths for columnar tables
			 * already (see related comments in TableAmRoutine). For this
			 * reason, we only consider IndexPath's here.
			 */
								...
								Oid relationId, IndexPath *indexPath)
{
	 ...
	 * we wouldn't read the same stripe again and again thanks to locality.
	 */
	 ...
	 * that, we use complement to 1 of absolute correlation, where being close
	 * to 0 means high correlation and being close to 1 means low correlation.
...
CheckVarStats(PlannerInfo *root, Var *var, Oid sortop, float4 *absVarCorrelation)
{
	...
	if (Abs(varCorrelation) < ColumnarQualPushdownCorrelationThreshold)
	{
		...
		{
			/*
			 * Report absVarCorrelation if caller wants to know why given var
			 * is rejected.
			 */
...
ExtractPushdownClause(PlannerInfo *root, RelOptInfo *rel, Node *node)
{
	...
	if (IsA(node, BoolExpr))
	{
		...
		if (boolExpr->boolop == NOT_EXPR)
		{
			 ...
			 * here, then it can't be a pushdownable one, such as: WHERE id
			 * NOT IN (SELECT id FROM something).
			 */
		...
		columnar_foreach_ptr(boolExprArg, boolExpr->args)
		{
	...
	/*
	 * Check that statistics on the Var support the utility of this clause.
	 */
...
Combinations(int n, int k)
{
	 ...
	 * close to n!, and we may overflow even if the input is reasonable (e.g.
	 * Combinations(500, 500)). Use formula (5) to choose the smaller, but
	 * equivalent, k.
	 */
	k = Min(k, n - k);
						...
						int depthLimit)
{
	...
	/*
	 * For columnar custom scan we should also do planning for parallel
	 * exeuction if it is enabled with GUC.
	 */
	if (columnar_enable_parallel_execution)
	{
		...
		/*
		 * We know how many workers are we going to run. Since workers are
		 * using single stripe probably it doesn't have much sense to spawn
		 * more workers than stripes in table.
		 */
					...
					Relids paramRelids)
{
	...
	{
		/*
		 * If perminfoindex > 0, that means that this relation's permission
		 * info is directly found in the list of rteperminfos of the
		 * Query(root->parse) So, all we have to do here is retrieve that
		 * info.
		 */
	...
	{
		/*
		 * If perminfoindex = 0, that means we are skipping the check for
		 * permission info for this relation, which means that it's either a
		 * partition or an inheritance child. In these cases, we need to
		 * access the permission info of the top parent of this relation.
		 * After thorough checking, we found that the index of the top parent
		 * pointing to the correct range table entry in Query's range tables
		 * (root->parse->rtable) is found under RelOptInfo
		 * rel->top_parent->relid. For reference, check
		 * expand_partitioned_rtentry and expand_inherited_rtentry PG
		 * functions
		 */
	 ...
	 * selectedCols so we should consider at least one column that needs read
	 * (for better cost calculation).
	 */
...
ColumnarPerStripeScanCost(RelOptInfo *rel, Oid relationId, int numberOfColumnsRead)
{
	...
	columnar_foreach_ptr(stripeMetadata, stripeList)
	{
	...
	/*
	 * When no stripes are in the table we don't have a count in
	 * maxColumnCount. To prevent a division by zero turning into a NaN we
	 * keep the ratio on zero. This will result in a cost of 0 for scanning
	 * the table which is a reasonable cost on an empty table.
	 */
	if (maxColumnCount == 0)
	{
		return 0;
	 ...
	 * cost to make our estimation scale compatible with postgres. Since we
	 * are calculating the cost for a single stripe here, we use seq page cost
	 * instead of random page cost. This is because, random page access only
	 * happens when switching between columns, which is pretty much
	 * neglactable.
	 */
								...
								List *custom_plans)
{
	...
	/*
	 * Vectorized QUAL execution. Split QUAL list into vectorized list stored
	 * in third position of custom_exprs list and rest of qual list (which
	 * can't be vectorized) in scan.plan.qual.
	 */
...
ColumnarScan_BeginCustomScan(CustomScanState *cscanstate, EState *estate, int eflags)
{
	...
	/*
	 * Vectorization is enabled if global variable is set and there is at
	 * least one filter which can be vectorized.
	 */
	...
	/*
	 * If we have pending changes that need to be flushed (row_mask after
	 * update/delete) or new stripe we need to to them here because sequential
	 * columnar scan can have parallel execution and updated are not allowed
	 * in parallel mode.
	 */
			   ...
			   ExecScanRecheckMtd recheckMtd)
{
	...
	/*
	 * No qual to check and no projection to do and vectorization is not
	 * enabled, just skip all the overhead and return the raw scan tuple.
	 */
	...
	for (;;)
	{
		...
		/*
		 * Get next tuple from vector tuple slot if vectorization exeuction is
		 * enabled and if we exhausted previous vector.
		 */
				 ...
				 columnarScanState->vectorization.vectorPendingRowNumber == 0)
		{
			...
			/*
			 * No qual, no vectorized qual, no projection but we need to
			 * return vector for vectorized aggregate
			 */
		...
		/*
		 * Get next tuple from vector tuple slot if vectorization execution is
		 * enabled.
		 */
		if (columnarScanState->vectorization.vectorizationEnabled)
		{
...
ColumnarScanNext(ColumnarScanState * columnarScanState)
{
	...
	{
		/*
		 * the columnar access method does not use the flags, they are
		 * specific to heap
		 */
		uint32		flags = 0;
	...
	if (vectorizationEnabled)
	{
		...
		columnar_foreach_int(attrIndex, columnarScanState->vectorization.attrNeededList)
		{
...
ColumnarScan_EndCustomScan(CustomScanState *node)
{
	...
	if (node->ss.ps.ps_ExprContext)
	{
		...
		node->ss.ps.ps_ExprContext = NULL;
	}
								 ...
								 void *coordinate)
{
	...
	/*
	 * Serialize scan snapshot for workers so they see changes if we have
	 * flushed stripe / row_mask during this scan.
	 */
...
ColumnarVarNeeded(ColumnarScanState * columnarScanState)
{
	...
	while ((bmsMember = bms_next_member(neededAttrSet, bmsMember)) >= 0)
	{
		...
		/*
		 * varlevelsup is used to figure out the (query) level of the Var that
		 * we are investigating. Since we are dealing with a particular
