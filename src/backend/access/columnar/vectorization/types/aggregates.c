...
Datum
vemptycount(PG_FUNCTION_ARGS)
{
	int64		arg = PG_GETARG_INT64(0);

	if (!columnar_vector_aggregate_execution)
		arg++;
...
static ArrayType *
Int64AggStateArray(FunctionCallInfo fcinfo)
{
...
Int128AggStateForTransition(FunctionCallInfo fcinfo)
{
...
vint2acc(PG_FUNCTION_ARGS)
{
	...
	transarray = Int64AggStateArray(fcinfo);
	...
	if (!columnar_vector_aggregate_execution)
	{
		...
		{
			transdata->N++;
			transdata->sumX += (int64) PG_GETARG_INT16(1);
		}
...
vint2larger(PG_FUNCTION_ARGS)
{
	...
	{
		if (!PG_ARGISNULL(1))
			maxValue = Max(maxValue, PG_GETARG_INT16(1));
...
vint2smaller(PG_FUNCTION_ARGS)
{
	...
	{
		if (!PG_ARGISNULL(1))
			minValue = Min(minValue, PG_GETARG_INT16(1));
...
vint4acc(PG_FUNCTION_ARGS)
{
	...
	if (!columnar_vector_aggregate_execution)
	{
		...
		{
			transdata->N++;
			transdata->sumX += (int64) PG_GETARG_INT32(1);
		}
		PG_RETURN_ARRAYTYPE_P(transarray);
	}
...
vint4larger(PG_FUNCTION_ARGS)
{
	...
	{
		if (!PG_ARGISNULL(1))
			maxValue = Max(maxValue, PG_GETARG_INT32(1));
...
vint4smaller(PG_FUNCTION_ARGS)
{
	...
	{
		if (!PG_ARGISNULL(1))
			minValue = Min(minValue, PG_GETARG_INT32(1));
...
vint8acc(PG_FUNCTION_ARGS)
{
	...
	state = Int128AggStateForTransition(fcinfo);
	if (!columnar_vector_aggregate_execution)
	{
		if (!PG_ARGISNULL(1))
		{
			state->N++;
			state->sumX += (int128) PG_GETARG_INT64(1);
		}
...
vint8larger(PG_FUNCTION_ARGS)
{
	...
	{
		if (!PG_ARGISNULL(1))
			maxValue = Max(maxValue, PG_GETARG_INT64(1));
...
vint8smaller(PG_FUNCTION_ARGS)
{
	...
	{
		if (!PG_ARGISNULL(1))
			minValue = Min(minValue, PG_GETARG_INT64(1));
...
vdatelarger(PG_FUNCTION_ARGS)
{
	...
	{
		if (!PG_ARGISNULL(1))
			maxValue = Max(maxValue, PG_GETARG_DATEADT(1));
		PG_RETURN_DATEADT(maxValue);
	}
...
vdatesmaller(PG_FUNCTION_ARGS)
{
	...
	{
		if (!PG_ARGISNULL(1))
			minValue = Min(minValue, PG_GETARG_DATEADT(1));
		PG_RETURN_DATEADT(minValue);
	}
