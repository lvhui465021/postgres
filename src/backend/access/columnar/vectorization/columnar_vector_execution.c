...
bool		columnar_vector_aggregate_execution = false;
...
GetVectorizedProcedureOid(Oid procedureOid, Oid *vectorizedProcedureOid)
{
	...
	{
		*vectorizedProcedureOid = InvalidOid;
		return false;
	}
	...
	vectorizedProcedureName = palloc0(originalProcedureNameLen + 2);
	...
	funcNames = list_make2(makeString("columnar"), makeString(vectorizedProcedureName));
			...
			case T_NullIfExpr:	/* struct-equivalent to OpExpr */
				{
					...
					/*
					 * Let's inspect argument rules and break if they don't
					 * match rules for vectorized execution.
					 */
