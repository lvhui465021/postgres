...
#define hash_agg_entry_size columnar_hash_agg_entry_size
#define hash_agg_set_limits columnar_hash_agg_set_limits
#define ExecEndAgg ColumnarExecEndAgg
#define ExecReScanAgg ColumnarExecReScanAgg
#define AggCheckCallContext ColumnarAggCheckCallContext
#define AggGetAggref ColumnarAggGetAggref
#define AggStateIsShared ColumnarAggStateIsShared
		...
		else
		{
			...
			if (aggstate->grp_firstTuple != NULL)
			{
				...
				for (;;)
				{
					...
					columnar_vector_aggregate_execution = true;
					PG_TRY();
					{
					...
					PG_FINALLY();
					{
						columnar_vector_aggregate_execution = false;
					}
					PG_END_TRY();
	...
	for (;;)
	{
		...
		if (entry != NULL)
		{
			...
			aggstate->hash_pergroup[batch->setno] =
				TupleHashEntryGetAdditional(perhash->hashtable, entry);
			advance_aggregates(aggstate);
		}
		else
		{
						  ...
						  Oid *inputTypes, int numArguments)
{
	...
	/*
	 * #if PG_VERSION_NUM < PG_VERSION_15 When combining states, we have no
	 * use at all for the aggregate function's transfn. Instead we use the
	 * combinefn.  In this case, the transfn and transfn_oid fields of
	 * pertrans refer to the combine function rather than the transition
	 * function. #else Set up infrastructure for calling the transfn.  Note
	 * that invtrans is not needed here. #endif
	 */
