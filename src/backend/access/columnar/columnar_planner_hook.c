...
static bool columnar_planner_initialized = false;
...
AggRefArgsExpressionMutator(Node *node, void *context)
{
	...
	if (IsA(node, OpExpr) || IsA(node, DistinctExpr) || IsA(node, NullIfExpr))
	{
		...
		{
			elog(ERROR, "cache lookup failed for operator %u", opExprNode->opno);
		}
...
void
columnar_planner_init(void)
{
	if (columnar_planner_initialized)
		return;

	columnar_planner_initialized = true;
