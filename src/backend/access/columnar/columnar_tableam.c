...
typedef struct ColumnarScanDescData
{
	...
	/* ANALYZE uses this page callback as the outer scan driver. */
	bool		analyzeNextBlockReturned;
...
typedef struct IndexFetchColumnarData
{
	...
	bool		is_select_query;	/* CustomIndexScan only gets planned with
									 * SELECT query */

	/*
	 * We initialize cs_readState lazily in the first
	 * columnar_index_fetch_tuple call. However, we want to do memory
	 * allocations in a sub MemoryContext of columnar_index_fetch_begin. For
	 * this reason, we store scanContext in columnar_index_fetch_begin.
	 */
	MemoryContext scanContext;
...
static bool columnar_tableam_initialized = false;
							...
							bool returnVectorizedTuple)
{
	 ...
	 * initialized read state. We assume that beginscan is called in a context
	 * that will last until end of scan.
	 */
	 ...
	 * the storage which we are reading, so we need to use the tuple
	 * descriptor of "slot" in first read.
	 */
				...
				bool allow_strat, bool allow_sync, bool allow_pagemode)
{
	...
	scan->analyzeNextBlockReturned = false;
}
						   ...
						   bool *call_again, bool *all_dead)
{
	...
	*call_again = false;
	...
	else if (stripeWriteState == STRIPE_WRITE_ABORTED)
	{
		 ...
		 * constraint violation. In that case, indexAM provides dirty snapshot
		 * to index_fetch_tuple callback.
		 */
	...
	else if (stripeWriteState == STRIPE_WRITE_IN_PROGRESS)
	{
		...
		{
			/*
			 * Stripe write is in progress and its entry is inserted by
			 * current transaction, so obviously it must be written by me.
			 * Since caller might want to use tupleslot datums for some
			 * reason, do another look-up, but this time by first flushing our
			 * writes.
			 *
			 * XXX: For index scan, this is the only case that we flush
			 * pending writes of the current backend. If we have taught reader
			 * how to read from WriteStateMap. then we could guarantee that
			 * index_fetch_tuple would never flush pending writes, but this
			 * seem to be too much work for now, but should be doable.
			 */
		...
		else
		{
			 ...
			 * flushed yet. Here we set all attributes of given tupleslot to
			 * NULL before returning true and expect the indexAM callback that
			 * called us --possibly to check against constraint violation--
			 * blocks until writer transaction commits or aborts, without
			 * requiring us to fill the tupleslot properly.
			 *
			 * XXX: Note that the assumption we made above for the tupleslot
			 * holds for "unique" constraints defined on "btree" indexes.
			 *
			 * For the other constraints that we support, namely: * exclusion
			 * on btree, * exclusion on hash, * unique on btree; we still need
			 * to fill tts_values.
			 *
			 ...
			 *
			 * This is annoying, but this also explains why this hack works
			 * for unique constraints on btree indexes, and also explains how
			 * we would never end up with "else" condition otherwise.
			 */
							 ...
							 TM_IndexDeleteOp *delstate)
{
	...
	if (delstate->bottomup)
	{
		 ...
		 * _bt_bottomupdel_pass, which in turn calls
		 * _bt_delitems_delete_check. And this function is okay with ndeltids
		 * being set to 0 by tableAM for bottom-up deletion.
		 */
		delstate->ndeltids = 0;
	...
	else
	{
		 ...
		 * case, so here we cannot do the same trick that we do for bottom-up
		 * deletion. See the assertion around table_index_delete_tuples call
		 * in pg function index_compute_xid_horizon_for_tuples.
		 *
					  ...
					  int options, BulkInsertState bistate)
{
	...
	/*
	 * columnar_init_write_state allocates the write state in a longer lasting
	 * context, so no need to worry about it.
	 */
								   ...
								   MultiXactId *minmulti)
{
	...
	/*
	 * If existing and new relfilelocator are different, that means the
	 * existing storage was dropped and we also need to clean up the metadata
	 * and state. If they are equal, this is a new relation object and we
...
columnar_relation_nontransactional_truncate(Relation rel)
{
	 ...
	 * transaction and no other transaction can see this relation yet. We can
	 * just truncate the relation.
	 *
								   ...
								   double *tups_recently_dead)
{
	...
	/*
	 * copy_table_data in cluster.c assumes tuple descriptors are exactly the
	 * same. Even dropped columns exist and are marked as attisdropped in the
	 * target relation.
	 */
...
TruncateAndCombineColumnarStripes(Relation rel, int elevel)
{
	...
	/*
	 * There is only one stripe that is candidate. Maybe we should vacuum it
	 * if condition is met.
	 */
	else if (startingStripeListPosition == 1)
	{
		/*
		 * Maybe we should vacuum only one stripe if count of deleted rows is
	 ...
	 * we write here into metadata heap tables. If concurrent process read
	 * page in which we inserted metadata tuples these tuples will be
	 * considered DEAD and will be removed (in problematic scenarion).
	 * Concurrent process, when assigning RecentXmin will scan all active
	 * processes in system but will NOT consider this process because of
	 * `PROC_IN_VACUUM` flag set. It looks that invalidating status flag here
	 * doesn't affect further execution.
	 */
					...
					BufferAccessStrategy bstrategy)
{
	 ...
	 * the relation in ColumnarMetapageCheckVersion. So if needed, upgrade the
	 * metapage before doing anything.
	 */
	bool		isUpgrade = true;
	...
	/*
	 * We don't have updates, deletes, or concurrent updates, so all we care
	 * for now is truncating the unused space at the end of storage.
	 */
	...
	foreach(stripeMetadataCell, stripeList)
	{
		...
		for (uint32 column = 0; column < skiplist->columnCount; column++)
		{
			...
			for (uint32 chunk = 0; chunk < skiplist->chunkCount; chunk++)
			{
				...
				/*
				 * We don't compress exists buffer, so its compressed &
				 * decompressed lengths are the same.
				 */
...
TruncateColumnar(Relation rel, int elevel)
{
	 ...
	 * lock).
	 *
	 * The decisions for AccessExclusiveLock and conditional lock with a
	 * timeout is based on lazy_truncate_heap in vacuumlazy.c.
	 */
	...
	{
		/*
		 * We failed to establish the lock in the specified number of retries.
		 * This means we give up truncating.
		 */
		ereport(elevel,
	...
	/*
	 * If we didn't truncate and combine tail stripes we could have stituation
	 * where we need to truncate storage at the end.
	 */
	if (!stripesTruncated)
	{

		/*
		 * Due to the AccessExclusive lock there's no danger that new stripes
		 * be added beyond highestPhysicalAddress while we're truncating.
		 */
#if PG_VERSION_NUM >= PG_VERSION_16
...
{
	ColumnarScanDesc columnarScan = (ColumnarScanDesc) scan;

	/*
	 * Our access method is not pages based, i.e. tuples are not confined to
	 * pages boundaries. Return one synthetic block so acquire_sample_rows()
	 * in analyze.c calls columnar_scan_analyze_next_tuple(), then stop the
	 * outer block loop after all tuples have been consumed.
	 */
	if (columnarScan->analyzeNextBlockReturned)
	{
		return false;
	}

	columnarScan->analyzeNextBlockReturned = true;
	return true;
}
								 ...
								 TupleTableSlot *slot)
{
	 ...
	 *
	 * We could do something like that here by choosing sample stripes or
	 * chunks, but getting that correct might need quite some work. Since
	 * columnar_fdw's ANALYZE scanned all rows, as a starter we do the same
	 * here and scan all rows.
	 */
								...
								TableScanDesc scan)
{
	...
	{
		/*
		 * Columnar utility hook already errors out for BRIN indexes on
		 * columnar tables, but be on the safe side.
		 */
	...
	/*
	 * Set up execution state for predicate, if any. Note that this is only
	 * useful for partial indexes.
	 */
...
{
	/*
	 * Indeed, columnar tables might have gaps between row numbers, e.g due to
	 * aborted transactions etc. Also, ItemPointer BlockNumber's for columnar
	 * tables don't actually correspond to actual disk blocks as in heapAM.
	 * For this reason, we call them as "virtual" blocks. At the moment, we
	 * believe it is better to report our progress based on this "virtual"
	 * block concept instead of doing nothing.
	 */
						  ...
						  ExprState *predicate)
{
	...
	while (columnar_getnextslot(scan, ForwardScanDirection, slot))
	{
		...
		{
			/*
			 * columnar_getnextslot guarantees that returned tuple will always
			 * have a greater ItemPointer than the ones we fetched before, so
			 * we directly use BlockNumber to report our progress.
			 */
		...
		{
			/*
			 * for partial indexes, discard tuples that don't satisfy the
			 * predicate
			 */
			continue;
		}
...
TupleSortSkipSmallerItemPointers(Tuplesortstate *tupleSort, ItemPointer targetItemPointer)
{
		   ...
		   ItemPointerCompare(&tsItemPointerData, targetItemPointer) < 0)
	{
		...
		/*
		 * If int8 is pass-by-ref, we need to free Datum memory. See
		 * tuplesort_getdatum function's comment.
		 */
...
{
	if (columnar_tableam_initialized)
		return;

	columnar_tableam_initialized = true;
...
{
	/*
	 * Lock relation to prevent it from being dropped and to avoid race
	 * conditions in the next if block.
	 */
	...
	{
		/*
		 * Drop metadata. No need to drop storage here since for tableam
		 * tables storage is managed by postgres.
		 */
 ...
 * access method. This can be called before columnar metadata is initialized.
 */
bool
IsColumnarTableAmTable(Oid relationId)
{
	...
	/*
	 * Lock relation to prevent it from being dropped & avoid race conditions.
	 */
...
HolesForRelation(Relation rel)
{
	...
	foreach(lc, stripeMetadataList)
	{
		...
		/*
		 * Arbitrary check to see if the offset will be large enough, 10000
		 * bytes is what was chosen.
		 */
 ...
 */
static void
vacuum_signal_handler(int signal)
{
...
vacuum_columnar_table(PG_FUNCTION_ARGS)
{
	 ...
	 * killing during a write could cause corruption.  Give us time to clean
	 * as much as we can up before letting the signal pass on.
	 */
	need_to_bail = false;
	last_signal = 0;
	...
	foreach(lc, stripeMetadataList)
	{
		...
		/*
		 * If inspected stripe has less than 0.5 percent of maximum strip row
		 * size or percentage of deleted rows is less than 20% we will skip
		 * this stripe for vacuum.
		 */
	...
	/*
	 * Combine the vacuum candidates into their own stripes appended to the
	 * rel, this should clear out any space from partial stripes to make space
	 * to move stripes into.
	 */
	...
	/*
	 * We have finished the first route of writes, let's drop our lock until
	 * we are ready for the next stage: compaction.
	 */
	ColumnarEndWrite(writeState);
	...
	/*
	 * Continually iterate through the holes, finding where we can place old
	 * stripes.
	 */
	bool		done = false;
