...
static Oid	LookupLocalTempRelidByRelfilenumber(Oid reltablespace,
...
{
	/*
	 * When upgrading we should retain the options from the previous cluster
	 * and don't write new options.
	 */
	...
	if (HeapTupleIsValid(heapTuple))
	{
		...
		{
			/*
			 * TODO check if the options are actually different, skip if not
			 * changed
			 */
...
DeleteColumnarTableOptions(Oid regclass, bool missingOk)
{
	...
	/*
	 * When upgrading we shouldn't delete or modify table options and retain
	 * options from the previous cluster.
	 */
	Assert(!IsBinaryUpgrade);
	...
	{
		/* columnar metadata is not available */
		return false;
	}
				   ...
				   TupleDesc tupleDescriptor)
{
	...
	SaveStripeSkipListForStorageId(storageId, stripe, chunkList, tupleDescriptor);
}


void
SaveStripeSkipListForStorageId(uint64 storageId, uint64 stripe, StripeSkipList * chunkList,
							   TupleDesc tupleDescriptor)
{
				...
				List *chunkGroupRowCounts)
{
	...
	SaveChunkGroupsForStorageId(storageId, stripe, chunkGroupRowCounts);
}


void
SaveChunkGroupsForStorageId(uint64 storageId, uint64 stripe,
							List *chunkGroupRowCounts)
{
				 ...
				 uint64 stripeStartRowNumber, List *chunkGroupRowCounts)
{
	...
	foreach(lc, chunkGroupRowCounts)
	{
		...
		for (uint16 n = 0; n < rowMaskIterations; n++)
		{
			...
			/*
			 * columnar.row_mask has UNIQUE constraint which can throw error
			 * so we need to catch this and return to function caller if
			 * saving empty row mask was succesful or not.
			 */
			PG_TRY();
			{
				InsertTupleAndEnforceConstraints(modifyState, values, nulls);
			  ...
			  Snapshot snapshot, uint64 rowNumber)
{
	...
	/*
	 * IF we have been blocked by advisory lock for storage, maybe row was
	 * delete by some other transaction.
	 */
...
void
FlushRowMaskCache(RowMaskWriteStateEntry * rowMaskEntry)
{
...
CheckStripeMetadataConsistency(StripeMetadata * stripeMetadata)
{
	...
	/*
	 * Even if stripe is flushed, fileOffset and dataLength might be equal to
	 * 0 for zero column tables, but those two should still be consistent with
	...
	{
		/*
		 * If stripe was flushed to disk, then we expect stripe to store at
		 * least one tuple.
		 */
		return;
	}
	...
	{
		/*
		 * If stripe was not flushed to disk, then values of given four fields
		 * should match the columns inserted by InsertEmptyStripeMetadataRow.
		 */
		return;
	}
						...
						Snapshot snapshot)
{
	...
	*chunkGroupDeletedRows = palloc0(chunkGroupCount * sizeof(uint32));
	...
	/*
	 * Since we have now updates of `chunk_group`, there could be multiple
	 * tuples retrieved with changed only deleted_row count. We expect that
	 * last modified version will be retrieved last so it we just update
	 * information based on tuple chunk group index.
	 */
	while (HeapTupleIsValid(heapTuple = systable_getnext_ordered(scanDescriptor,
																 ForwardScanDirection)))
	{
				   ...
				   uint64 stripeRowCount)
{
	...
	/*
	 * XXX: Instead of inserting a dummy entry to columnar.stripe and updating
	 * it when flushing the stripe, we could have a hash table in shared
	 * memory for the bookkeeping of ongoing writes.
	 */
						...
						Datum *newValues)
{
	...
	void	   *inplaceState = NULL;

	systable_inplace_update_begin(columnarStripes, ColumnarStripePKeyIndexRelationId(),
								  true, NULL, 2, scanKey, &oldTuple,
								  &inplaceState);
	...
	systable_inplace_update_finish(inplaceState, modifiedTuple);

	/*
	 * Existing tuple now contains modifications, because we used an in-place
	 * update.
	 */
...
BuildStripeMetadata(Relation columnarStripes, HeapTuple heapTuple)
{
	...
	/*
	 * If there is unflushed data in a parent transaction, then we would have
	 * already thrown an error before starting to scan the table.. If the data
	 * is from an earlier subxact that committed, then it would have been
	 * flushed already. For this reason, we don't care about subtransaction id
	 * here.
	 */
...
{
	/*
	 * We copy the data so the result of this function lives even after the
	 * byteaDatum is freed.
	 */
...
LookupStorageId(RelFileLocator relfilelocator)
{
	...
	{
		relationId =
			LookupLocalTempRelidByRelfilenumber(RelationTablespace_compat(relfilelocator),
												RelationPhysicalIdentifierNumber_compat(
																						relfilelocator));
	}
...
static Oid
LookupLocalTempRelidByRelfilenumber(Oid reltablespace, RelFileNumber relfilenumber)
{
	ScanKeyData scanKey[2];
	SysScanDesc scanDescriptor;
	Relation	relation;
	...
	ScanKeyInit(&scanKey[0], Anum_pg_class_reltablespace,
				BTEqualStrategyNumber, F_OIDEQ, ObjectIdGetDatum(reltablespace));
	ScanKeyInit(&scanKey[1], Anum_pg_class_relfilenode,
				BTEqualStrategyNumber, F_OIDEQ, ObjectIdGetDatum(relfilenumber));
	...
	{
		Form_pg_class classForm = (Form_pg_class) GETSTRUCT(heapTuple);
		...
		if (!isTempOrTempToastNamespace(classForm->relnamespace))
		{
			continue;
		}

		relationId = classForm->oid;
		break;
	}
...
GetHighestUsedRowNumber(uint64 storageId)
{
	...
	columnar_foreach_ptr(stripeMetadata, stripeMetadataList)
	{
