...
struct ColumnarReadState
{
	...
	/*
	 * Memory context guaranteed to be not freed during scan so we can safely
	 * use for any memory allocations regarding ColumnarReadState itself.
	 */
...
{
	/*
	 * We allocate all stripe specific data in the stripeReadContext, and
	 * reset this memory context before loading a new stripe. This is to avoid
	 * memory leaks.
	 */
	...
	if (!randomAccess)
	{
		...
		{
			/*
			 * Flush any pending in-memory changes to row_mask metadata for
			 * next scan.
			 */
			...
			/*
			 * When doing random access (i.e.: index scan), we don't need to
			 * flush pending writes until we need to read them.
		 ...
		 * snapshot, but AdvanceStripeRead would throw an error for that. Note
		 * that this is not the case with for plain table scan methods (i.e.:
		 * SeqScan and Columnar CustomScan).
		 *
...
AdvanceStripeRead(ColumnarReadState * readState)
{
	...
	else
	{
		...
		/*
		 * There exists higher stripe id than this one so adjust and add +1
		 * for next workers.
		 */
					  ...
					  bool *columnNulls, int32 *deletedColumnsNumber)
{
	...
	while (chunkGroupReadState->currentRow < chunkGroupReadState->rowCount)
	{
		...
		columnar_foreach_int(attno, chunkGroupReadState->projectedColumnList)
		{
				  ...
				  List *whereClauseVars, int64 *chunkGroupsFiltered)
{
	...
	foreach(columnCell, whereClauseVars)
	{
		...
		for (chunkIndex = 0; chunkIndex < stripeSkipList->chunkCount; chunkIndex++)
		{
			...
			/*
			 * A column chunk with comparable data type can miss min/max
			 * values if all values in the chunk are NULL.
			 */
...
{
	/*
	 * We don't recurse into or include aggregates, window functions, or PHVs.
	 * We don't expect any PHVs during execution; and Vars found inside an
	 * aggregate or window function aren't going to be useful in forming
...
ProjectedColumnMask(uint32 columnCount, List *projectedColumnList)
{
	...
	columnar_foreach_int(attno, projectedColumnList)
	{
					 ...
					 List *projectedColumnList, StripeReadState * state, uint64 stripeId)
{
	...
	for (columnIndex = 0; columnIndex < stripeBuffers->columnCount; columnIndex++)
	{
		...
		if (columnBuffers != NULL)
		{
			...
			/*
			 * store current chunk's data buffer to be freed at next chunk
			 * read
			 */
					 ...
					 uint64 stripeFirstRowNumber)
{
	...
	while (true)
	{
		...
		{
			/*
			 * if *newVectorSize is non-zero, it means there might be some
			 * data in chunkGroupReadState->chunkGroupData referenced by
			 * VectorColumn.  In this case, do NOT release
			 * ChunkGroupReadState.
			 */
			if (*newVectorSize == 0)
			{
				/*
				 * if this chunk group is exhausted, fetch the next one and
				 * loop
				 */
				EndChunkGroupRead(stripeReadState->chunkGroupReadState);
				stripeReadState->chunkGroupReadState = NULL;
	...
	for (i = 0; i < chunkGroupReadState->rowCount; i++)
	{
		...
		columnar_foreach_int(attno, chunkGroupReadState->projectedColumnList)
		{
			...
			if (chunkGroupData->existsArray[columnIndex][rowIndex])
			{
				...
				/*
				 * For data types which have len less or equal 8 we can use
				 * `store_att_byval` function.
				 */
