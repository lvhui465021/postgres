...
struct ColumnarWriteState
{
	...
	/*
	 * compressionBuffer buffer is used as temporary storage during data value
	 * compression operation. It is kept here to minimize memory allocations.
	 * It lives in stripeWriteContext and gets deallocated when memory context
	 * is reset.
	 */
...
static Relation OpenWriteStateRelation(ColumnarWriteState * writeState, bool *relationOpened);
static void CloseWriteStateRelation(Relation relation, bool relationOpened);
				   ...
				   TupleDesc tupleDescriptor)
{
	...
	writeState->useRelationForStorage =
		relation->rd_rel->relpersistence == RELPERSISTENCE_TEMP;
	writeState->storageId = ColumnarStorageGetStorageId(relation, false);
...
ColumnarWriteRow(ColumnarWriteState * writeState, Datum *columnValues, bool *columnNulls)
{
	...
	if (stripeBuffers)
	{
		...
		/*
		 * For each column, we first need to check to see if the next row will
		 * fit inside the chunk buffer.  If it does not fit, then we need to
		 * serialize the stripe and make a new stripe for insertion.
		 */
		bool		fits = true;
	...
	if (stripeBuffers == NULL)
	{
		...
		relation = OpenWriteStateRelation(writeState, &relationOpened);
		writeState->emptyStripeReservation =
			ReserveEmptyStripe(relation, columnCount, chunkRowCount,
							   options->stripeRowCount);
		CloseWriteStateRelation(relation, relationOpened);

		/*
		 * serializedValueBuffer lives in stripe write memory context so it
		 * needs to be initialized when the stripe is created.
		 */
		for (columnIndex = 0; columnIndex < columnCount; columnIndex++)
		{
...
ColumnarFlushPendingWrites(ColumnarWriteState * writeState)
{
	...
	if (stripeBuffers != NULL)
	{
		...
		/*
		 * set stripe data and skip list to NULL so they are recreated next
		 * time
		 */
		writeState->stripeBuffers = NULL;
		writeState->stripeSkipList = NULL;
...
FlushStripe(ColumnarWriteState * writeState)
{
	...
	uint32		chunkIndex = 0;
	bool		relationOpened = false;
	Relation	relation = OpenWriteStateRelation(writeState, &relationOpened);
	StripeBuffers *stripeBuffers = writeState->stripeBuffers;
	StripeSkipList *stripeSkipList = writeState->stripeSkipList;
	...
	/*
	 * check if the last chunk needs serialization , the last chunk was not
	 * serialized if it was not full yet, e.g.  (rowCount > 0)
	 */
	...
	/*
	 * Each stripe has only one section: Data section, in which we store data
	 * for each column continuously. We store data for each for each column in
	 * chunks. For each chunk, we store two buffers: "exists" buffer, and
	 * "value" buffer. "exists" buffer tells which values are not NULL.
	 * "value" buffer contains values for present values. For each column, we
	 * first store all "exists" buffers, and then all "value" buffers.
	 */
	...
	SaveChunkGroupsForStorageId(writeState->storageId,
								stripeMetadata->id,
								writeState->chunkGroupRowCounts);
	SaveStripeSkipListForStorageId(writeState->storageId,
								   stripeMetadata->id,
...
static Relation
OpenWriteStateRelation(ColumnarWriteState * writeState, bool *relationOpened)
{
	if (writeState->useRelationForStorage)
	{
		*relationOpened = false;
		return writeState->relation;
	}
...
static void
CloseWriteStateRelation(Relation relation, bool relationOpened)
{
	if (relationOpened)
	{
...
SerializeChunkData(ColumnarWriteState * writeState, uint32 chunkIndex, uint32 rowCount)
{
	...
	for (columnIndex = 0; columnIndex < columnCount; columnIndex++)
	{
		...
		/*
		 * if serializedValueBuffer is be compressed, update
		 * serializedValueBuffer with compressed data and store compression
		 * type.
		 */
