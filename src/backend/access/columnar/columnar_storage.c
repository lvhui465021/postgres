...
{
	/*
	 * Store version of file format used, so we can detect files from previous
	 * versions if we change file format.
	 */
	uint32		versionMajor;
	uint32		versionMinor;

	/*
	 * Each of the metadata table rows are identified by a storageId. We store
	 * it also in the main fork so we can link metadata rows with data files.
	 */
	uint64		storageId;
...
ColumnarStorageTruncate(Relation rel, uint64 newDataReservation)
{
	...
	/*
	 * Truncate the storage. Note that RelationTruncate() takes care of Write
	 * Ahead Logging.
	 */
...
ColumnarMetapageRead(Relation rel, bool force)
{
	...
	{
		/*
		 * We only expect this to happen with columnar files created by older
		 * extension versions. In the current in-core version, we immediately
		 * create the metapage for columnar tables, i.e right after creating
		 * the table. However in older versions, we were creating metapages
		 * lazily, i.e when ingesting data to columnar table.
		 */
	...
	/*
	 * Regardless of "force" parameter, always force read metapage block. We
	 * will check metapage version in ColumnarMetapageCheckVersion depending
	 * on "force".
	 */
			 ...
			 uint32 len, bool clear)
{
	...
	/*
	 * After a transaction has been rolled-back, we might be over-writing the
	 * rolledback write, so phdr->pd_lower can be different from addr.offset.
	 *
	 * We reset pd_lower to reset the rolledback write.
	 *
	 * Given that we always align page reservation to the next page as of
	 * 10.2, having such a disk page is only possible if write operaion failed
	 * in an older version of columnar, but now user attempts writing to that
	 * table in version >= 10.2.
	 */
